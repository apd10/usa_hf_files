...
if att_cfg_file is not None:
    with open(att_cfg_file) as stream:
...
def train_step(layer_idx, key_states, query_states, attn_weights, topk):
    K = key_states.to("cuda:1").float()
    Q = query_states.to("cuda:1").float()
    k = K.shape[2]
    q = Q.shape[2]
...
def approximate_attention(attn_weights, layer_idx, hidden_states, keys, queries):
    ...
    b,a,q,k = attn_weights.shape
    if  att_cfg["mode"] == "local":
        num = att_cfg["local"]["num"]
        mask = torch.tril(torch.ones(q,k,device=attn_weights.device), diagonal=k-q-num) * -3.3895e+38
        attn_weights = attn_weights + mask.unsqueeze(0).unsqueeze(0)
    elif att_cfg["mode"] == "first":
        num = att_cfg["first"]["num"]
        mask = torch.tril(torch.ones(q,k,device=attn_weights.device), diagonal=k-q)
        mask[:,:num] = 0.
        mask = mask * -3.3895e+38
        attn_weights = attn_weights + mask
    elif att_cfg["mode"] == "local_and_first":
        ...
        if ("skip_layers" in att_cfg.keys() 
            ...
            values, idx = torch.topk(to_be_masked_att, att_cfg["top"]["num"], dim=-1)
            view_idx = idx.view(-1,idx.shape[-1])
            view_idx = view_idx + torch.arange(view_idx.shape[0], device=view_idx.device).reshape(-1,1) * attn_weights.shape[-1]
